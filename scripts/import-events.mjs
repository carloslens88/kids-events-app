...
// No hay límite documentado en este dominio, pero tampoco tiene sentido
// lanzar cientos de peticiones a la vez: unas pocas en paralelo a la vez.
...
async function fetchZaragozaEvents() {
  const rows = [];
...
const zaragoza = await fetchSafely('Agenda de Zaragoza', fetchZaragozaEvents);
console.log(`Agenda de Zaragoza: ${zaragoza.length} eventos infantiles/juveniles en ventana`);

const allRows = [...madrid, ...eventbrite, ...barcelona, ...malaga, ...zaragoza];
